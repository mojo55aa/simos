...
#define ONE_PAGE_DIR 0x400000   /*一个页目录项对应4MB内存*/
...
/**
 * get_kernel_pages 返回内核空间页数,同mem_pool_init的前部分
 * @total_mem: 总内存数
 * return: 内核页表数
*/
...
void page_table_init(uint32_t kernel_pages)
{
    ...
    if ((kernel_pages * PAGE_SIZE) % ONE_PAGE_DIR == 0) /*总内存字节数除以4MB,余数为0*/
    {
        page_dir_num = (kernel_pages * PAGE_SIZE) / ONE_PAGE_DIR;
    ...
    else    /*余数不为0，需要计算最后一个页目录项对应的页表项数*/
    {
        ...
        last_page_table_num = ((kernel_pages * PAGE_SIZE) - (ONE_PAGE_DIR * (kernel_pages - 1))) / PAGE_SIZE;
    }

    uint16_t kernel_page_mark = 0x0 | PG_P_ENABLE | PG_RW_W | PG_US_S; /*kernel页表属性，存在 可写 超级用户*/
    uint32_t i;
    for (i = 0; i < 768; i++)    /*前168项，3GB空间不映射*/
    {
        page_dir_addr[i] = 0x0;
    }
    ...
    {
        /*每一个页目录项*/
        uint32_t each_page_dir = page_table_base + i + kernel_page_mark;
